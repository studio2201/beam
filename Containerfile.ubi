# =============================================================================
# beam — Alpine container image (Unraid-compatible)
# =============================================================================
...
#     UPLOAD_DIR env var if needed.
#   - Listens on 0.0.0.0:<port> for bridge networking.
#
# Build (Podman):
#   cd ~/projects/studio2201/beam
#   podman build -f Containerfile.ubi -t beam:latest .
#
# Run (Unraid-style):
#   podman run -d --name beam \
#     -p <host-port>:4401 \
#     -v /mnt/user/appdata/beam:/config \
#     -e BEAM_PIN=<your-pin> \
#     beam:latest
#

# Compare size:
#   podman images | grep -E 'beam|studio2201/beam'
# =============================================================================
...
LABEL org.opencontainers.image.title="beam" \
      org.opencontainers.image.description="beam (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/beam" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/beam
RUN chmod +x /usr/local/bin/beam
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4401}/health" >/dev/null || exit 1
